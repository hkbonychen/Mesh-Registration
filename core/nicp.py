...
try:
    ...
    def spsolve(sparse_X, dense_b):
        ...
        with stdout_redirected():
            factor = cholesky_AAt(sparse_X.T, mode = "auto", ordering_method="default", use_long=None)
...
def non_rigid_icp_generator(source, target, eps=1e-3,
    ...
    lio.ply_from_array('aligned_source', 0, source, './output/', True, 'unre_pickpoints', [1,0,1,0,1,0])
    lio.ply_from_array('aligned_target', 0, target, './output/', True, 'unre_pickpoints',[1,0,1,0,1,0])
    ...
    if stiffness_weights is not None:
        ...
        stiffness_weights = [50, 20, 5, 2, 0.8, 0.5, 0.35, 0.2] #original
        #stiffness_weights = [50, 20, 5, 2, 0.8]
        #stiffness_weights = [50, 40, 30, 20, 5, 2, 0.8, 0.5, 0.35, 0.2, 0.1, 0.05]
        #stiffness_weights = [ 10, 5]
        if verbose:
            print('using default '
    ...
    if landmark_weights is not None:
        ...
        # these values have been empirically found to perform well for well
        # rigidly aligned facial meshes
        landmark_weights = [5, 2, .5, 0, 0, 0, 0, 0] #original
        #landmark_weights = [0, 0, 0, 0, 0]
        #landmark_weights = [15, 12, 10,  8, 4, .5, 0.1, 0, 0, 0, 0, 0]
        #landmark_weights = [5, 2]
        if verbose:
            print('using default '
                  'landmark_weights: {}'.format(landmark_weights))
    else:
