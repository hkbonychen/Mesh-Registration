...
def generate_data_weights_per_iter(template, nosetip, r_width, w_min_iter,
    ...
    # stiffness weights
    stiffness_weights = np.array([50, 20, 5, 2, 0.8, 0.5, 0.35, 0.2]) #original
...
def correspond_mesh(template, mesh, mask=None, verbose=False, landmark_type = 'ibug68'):
    group = '_lsfm'
    if hasattr(mesh, 'texture'):
        if mask is not None:
            template.landmarks['__lsfm_masked'] = template.landmarks['__lsfm'].lms.from_mask(mask)
            group = '__lsfm_masked'
        if verbose:
            n_bad_lms = (~mask).sum()
            if not n_bad_lms == 0:
        ...
        # codes to load landmark from external file and overwrite the mesh.landmarks points with the new one
        # modified by Bony on 18-07-2019        
        group = 'unre_pickpoints'
        # modified by Bony on 18-07-2019

    aligned = non_rigid_icp(template, mesh, landmark_group=group, eps=1e-3,
